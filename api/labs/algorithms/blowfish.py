...
class Blowfish(object):
    def __init__(self, key, byte_order="big",
                 P_array=PI_P_ARRAY, S_boxes=PI_S_BOXES):
        if not 4 <= len(key) <= 56:
    ...
    def execute(self, input: io.BufferedReader, output: io.BufferedWriter,
                decrypt: bool, mode: str, init_vector: bytes = None,
    ...
    @staticmethod
    def _encrypt(L, R, P, S1, S2, S3, S4):
        for p1, p2 in P[:-1]:
            L ^= p1
            a, b, c, d = struct.unpack("=4B", struct.pack(">I", L))
            R ^= (S1[a] + S2[b] ^ S3[c]) + S4[d] & 0xffffffff
            R ^= p2
            a, b, c, d = struct.unpack("=4B", struct.pack(">I", R))
    ...
    def _encrypt_block(self, block):
        ...
        p_penultimate, p_last = P[-1]
        return struct.pack(">2I", R ^ p_last, L ^ p_penultimate)
    ...
    def _decrypt_block(self, block):
        ...
        return struct.pack(">2I", R ^ p_first, L ^ p_second)

    def _encrypt_ecb(self, input, output, processes, blocks_per_process):
        pool = multiprocessing.Pool(processes)
        span_size = processes * blocks_per_process * self.block_size

        while len(data := input.read(span_size)) == span_size:
            blocks = self._split_equally(data, self.block_size)
            for block in pool.imap(self._encrypt_block, blocks,
                                   blocks_per_process):
                output.write(block)

        padded = self._pad_end(data)
        blocks = self._split_equally(padded, self.block_size)
        for block in pool.imap(self._encrypt_block, blocks,
                               blocks_per_process):
            output.write(block)
    ...
    def _encrypt_cbc(self, input, output, init_vector: bytes):
        encrypted_block = init_vector.ljust(self.block_size, b'\x00')

        while len(data := input.read(self.block_size)) == self.block_size:
    ...
    def _decrypt_cbc(self, input, output, init_vector):
        decrypted_block = b""
        prev_block = init_vector.ljust(self.block_size, b'\x00')

        while len(data := input.read(self.block_size)):
            output.write(decrypted_block)
    ...
    def _pad_end(self, data):
        zeros = self.block_size - (len(data) % self.block_size) - 1
